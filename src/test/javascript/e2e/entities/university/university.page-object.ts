...
export default class UniversityComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('university-heading'));
...
export class UniversityDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('lfiErfassungBackendApp.university.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-university'));
}
