...
import com.soprasteria.lfi_erfassung_backend.config.KafkaProperties;
...
class LfiErfassungBackendKafkaResourceIT {
    ...
    void setup() {
        ...
        LfiErfassungBackendKafkaResource kafkaResource = new LfiErfassungBackendKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/lfi-erfassung-backend-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/lfi-erfassung-backend-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
